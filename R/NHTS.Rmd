...
```{r normal, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.CA <- NHTS.list$NHTS.df %>% filter(location %in% c("CA")) %>% group_by(location, mode, id) %>% summarise( T = sum(duration) ) %>% filter( mode %in% c("walk","cycle") ) %>% spread(mode, T, fill = 0) %>% filter( (walk > 1) | (cycle > 0)) %>% mutate(logTA = log(7*(3*walk + 6*cycle)))

ggqqplot(qq.CA$logTA)
```
```{r normal2, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.CA <- within(qq.CA,{
    logTA <- logTA - rnorm(n = length(logTA), mean = 3, sd = 0.5)
})
