...
trippub <- with(trippub,{
    data.frame(HOUSEID = as.character(HOUSEID),
               ...
               TRVLCMIN = ifelse(as.numeric(TRVLCMIN) < 0, as.numeric(NA), as.numeric(TRVLCMIN)),
...
perpub <- with(perpub,{
                    data.frame(houseid = as.character(HOUSEID),
                               ...
                               AGE = ifelse(as.numeric(R_AGE) < 0, as.numeric(NA), as.numeric(R_AGE)),
...
```{r TravelSurvey, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE}
perpub <- perpub %>% select(houseID, subjectID, sex, age)
hhpub <- hhpub %>% select(houseID, location = HHSTATE)

NHTS <- new("TravelSurvey", person = perpub, trip = trippub, house = hhpub)
saveRDS(NHTS, file = "./data/NHTS.ts.rds")
```

```{r join, eval = TRUE, echo = FALSE, results = "show", warning = FALSE, error = FALSE, message = FALSE, results="asis"}
