...
```{r trippub, eval = !RDS, echo = FALSE, results = "show", warning = FALSE, error = FALSE, message = FALSE, results="asis"}
...
```{r perpub, eval = !RDS, echo = FALSE, results = "show", warning = FALSE, error = FALSE, message = FALSE, results="asis"}
...
```{r hhpub, eval = !RDS, echo = FALSE, results = "show", warning = FALSE, error = FALSE, message = FALSE, results="asis"}
...
hhpub <- with(hhpub,{
                    ...
                    HHSTATE = factor(as.character(HHSTATE))
                               )
})
...
```{r join, eval = !RDS, echo = FALSE, results = "show", warning = FALSE, error = FALSE, message = FALSE, results="asis"}
...
```{r readRDS, eval = RDS, echo = FALSE, results = "show", warning = FALSE, error = FALSE, message = FALSE, results="asis"}
NHTS.list <- readRDS(file = "./R/data/NHTS.list.rds")
```

## Sample Size

First we present the sample sizes (persons) stratified by state.  Note
the log scale.  CA, TX, NY, WI, GA, NC, SC each has a sample size
greater than 10,000.
...
```
```{r histPerpub, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis", fig.height = 10}
...
Next we look at the overall national mode-share.  We use total
duration (minutes) per mode.  Note the log scale.  Car, SUV and pickup
truck are the top three with van in fifth place, just behind walk.

```{r modeShare, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis", fig.width = 8}
...
The household survey poses the question how frequently does the house
use walk/bike/car/etc.  Here we display the proportion of households
that responded "never" to walk and to bike.

```{r neverWalk, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis", fig.height = 20, fig.width = 10}
...
```{r WALK, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis", fig.height = 20, fig.width = 10}
...
```{r BIKE, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis", fig.height = 20, fig.width = 10}
...
```{r nMSA, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis", fig.height = 20, fig.width = 10}
...
```{r normalCA, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.CA <- NHTS.list$NHTS.df %>% filter(location %in% c("CA")) %>% group_by(location, mode, id) %>% summarise( T = sum(duration) ) %>% filter( mode %in% c("walk","cycle") ) %>% spread(mode, T, fill = 0) %>% filter( (walk > 0) | (cycle > 0)) %>% mutate(logTA = log(7*(3*walk + 6*cycle)))

ggqqplot(qq.CA$logTA)
```
```{r normal2CA, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.CA <- within(qq.CA,{
...
```{r normalWI, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.WI <- NHTS.list$NHTS.df %>% filter(location %in% c("WI")) %>% group_by(location, mode, id) %>% summarise( T = sum(duration) ) %>% filter( mode %in% c("walk","cycle") ) %>% spread(mode, T, fill = 0) %>% filter( (walk > 0) | (cycle > 0)) %>% mutate(logTA = log(7*(3*walk + 6*cycle)))

ggqqplot(qq.WI$logTA)
```
```{r normal2WI, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.WI <- within(qq.WI,{
    logTA <- logTA - rnorm(n = length(logTA), mean = 3, sd = 1)
})
...
```{r normalNY, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.NY <- NHTS.list$NHTS.df %>% filter(location %in% c("NY")) %>% group_by(location, mode, id) %>% summarise( T = sum(duration) ) %>% filter( mode %in% c("walk","cycle") ) %>% spread(mode, T, fill = 0) %>% filter( (walk > 0) | (cycle > 0)) %>% mutate(logTA = log(7*(3*walk + 6*cycle)))

ggqqplot(qq.NY$logTA)
```
```{r normal2NY, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.NY <- within(qq.NY,{
    logTA <- logTA - rnorm(n = length(logTA), mean = 3, sd = 1)
})
...
```{r normalTX, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.TX <- NHTS.list$NHTS.df %>% filter(location %in% c("TX")) %>% group_by(location, mode, id) %>% summarise( T = sum(duration) ) %>% filter( mode %in% c("walk","cycle") ) %>% spread(mode, T, fill = 0) %>% filter( (walk > 0) | (cycle > 0)) %>% mutate(logTA = log(7*(3*walk + 6*cycle)))

ggqqplot(qq.TX$logTA)
```
```{r normal2TX, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.TX <- within(qq.TX,{
    logTA <- logTA - rnorm(n = length(logTA), mean = 3, sd = 1)
})
...
```{r normalGA, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.GA <- NHTS.list$NHTS.df %>% filter(location %in% c("GA")) %>% group_by(location, mode, id) %>% summarise( T = sum(duration) ) %>% filter( mode %in% c("walk","cycle") ) %>% spread(mode, T, fill = 0) %>% filter( (walk > 0) | (cycle > 0)) %>% mutate(logTA = log(7*(3*walk + 6*cycle)))

ggqqplot(qq.GA$logTA)
```
```{r normal2GA, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.GA <- within(qq.GA,{
    logTA <- logTA - rnorm(n = length(logTA), mean = 3, sd = 1)
})
...
```{r normalNC, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.NC <- NHTS.list$NHTS.df %>% filter(location %in% c("NC")) %>% group_by(location, mode, id) %>% summarise( T = sum(duration) ) %>% filter( mode %in% c("walk","cycle") ) %>% spread(mode, T, fill = 0) %>% filter( (walk > 0) | (cycle > 0)) %>% mutate(logTA = log(7*(3*walk + 6*cycle)))

ggqqplot(qq.NC$logTA)
```
```{r normal2NC, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.NC <- within(qq.NC,{
    logTA <- logTA - rnorm(n = length(logTA), mean = 3, sd = 1)
})
...
```{r normalSC, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.SC <- NHTS.list$NHTS.df %>% filter(location %in% c("SC")) %>% group_by(location, mode, id) %>% summarise( T = sum(duration) ) %>% filter( mode %in% c("walk","cycle") ) %>% spread(mode, T, fill = 0) %>% filter( (walk > 0) | (cycle > 0)) %>% mutate(logTA = log(7*(3*walk + 6*cycle)))

ggqqplot(qq.SC$logTA)
```
```{r normal2SC, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results="asis"}
qq.SC <- within(qq.SC,{
    logTA <- logTA - rnorm(n = length(logTA), mean = 3, sd = 1)
})
...
```{r activityFile, eval = TRUE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE}
logTA.df <- NHTS.list$NHTS.df %>%
    ...
    mutate(logTA = log(7*(3*walk + 6*cycle))) %>%
    group_by(location) %>%
...
melt(data.frame(state = rownames(PAF.df), PAF.df)) %>% ggplot(aes(x = state, y = value, colour = variable)) + geom_point()
