...
trippub <- read.csv(file = "./data/trippub.csv", stringsAsFactors = FALSE)
...
perpub <- with(perpub,{
    data.frame(houseID=HOUSEID, subjectID=PERSONID, sex = SEX, age = AGE)
    })
...
NHTS <- readRDS(file = "./data/NHTS.ts.rds")
person.ID <- NHTS@person %>% mutate(ID = paste(houseID, subjectID, sep = "-")) %>% select(ID)
trip.ID <- NHTS@trip %>% mutate(ID = paste(houseID, subjectID, sep = "-")) %>% select(ID)
inner_join(getTA(NHTS), getp0(NHTS), by = "location")
```

```{r TravelSurvey3, eval = TRUE, echo = TRUE, warning = FALSE, error = FALSE, message = FALSE}
hhpub.meta <- hhpub.raw %>% select(houseID = HOUSEID, BIKE, WALK) %>% mutate(houseID = as.integer(houseID))

hhpub.meta <- within(hhpub.meta,{
    WALK = factor(ifelse(WALK %in% c(-9,-8,-7), NA, WALK), levels = 1:5, labels = c("daily","weekly","monthly","yearly","never"))
    BIKE = factor(ifelse(BIKE %in% c(-9,-8,-7), NA, BIKE), levels = 1:5, labels = c("daily","weekly","monthly","yearly","never"))
})

inner_join(NHTS@house, hhpub.meta, by = "houseID") %>% group_by(location) %>% count(WALK, BIKE) %>% ungroup() %>% mutate(WALK = paste0("walk-",WALK), BIKE = paste0("bike-",BIKE))
